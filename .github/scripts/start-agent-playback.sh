#!/usr/bin/env bash
# Start only a dedicated test server. The checkout, action and recordings must
# all use the same Conductor feature branch from agents-playback.yml.
set -euo pipefail
conductor_dir=$(cd "${1:?Usage: start-agent-playback.sh CONDUCTOR_CHECKOUT}" && pwd)
...
python3 spec/support/agents/http_fixture.py > "$playback_dir/http.log" 2>&1 &
echo $! > "$playback_dir/http.pid"
mcp-testkit --transport http --auth playback-test-key > "$playback_dir/mcp.log" 2>&1 &
