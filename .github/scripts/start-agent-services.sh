...
conductor_dir=$(cd "${1:?Usage: start-agent-services.sh CONDUCTOR_CHECKOUT [--check]}" && pwd)
cd "$repo_dir"
export CONDUCTOR_RECORDINGS_DIR="$conductor_dir/llm-recordings"
playback_dir=${CONDUCTOR_PLAYBACK_WORK_DIR:-"$repo_dir/tmp/agent-playback"}
...
check_services() {
  ...
  curl --fail --silent --show-error --max-time 3 \
    ...
    --data '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"ruby-sdk-playback","version":"1"}}}' \
...
nohup python3 spec/support/agents/http_fixture.py < /dev/null > "$playback_dir/http.log" 2>&1 &
pids+=("$!")
