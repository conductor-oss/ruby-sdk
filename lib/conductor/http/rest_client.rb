    ...
    class RestClient
      ...
      def request(method, url, query: nil, headers: nil, body: nil)
        ...
        begin
          ...
          # handle_response has already raised after constructing a proper status code;
          # re-raise after emitting telemetry.
          emit_http_event(method, url, status_code, start_time)
          raise
        end
      end
      ...
      # Publish HttpApiRequest event to both per-instance and global dispatchers.
      # The uri label carries only the request path (not the full URL) to match
      # the canonical spec and keep label cardinality manageable.
